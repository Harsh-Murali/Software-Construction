...
# Test Example: Testing the delete command with '4d' on sequence 1-5
echo "Testing delete command with '4d' on sequence 1-5"
...
seq 1 5 | python3 "$current_dir/eddy.py" '4d' > "$actual_output"

# Check the output against expected output
if ! diff "$expected_output" "$actual_output" > /dev/null
then
    echo "Failed test: Output does not match expected for '4d'"
    exit 1
fi
